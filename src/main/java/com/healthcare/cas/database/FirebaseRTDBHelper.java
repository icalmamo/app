...
public class FirebaseRTDBHelper {
    ...
    /**
     * Delete employee (HARD DELETE - permanently removes from Firebase)
     * This completely removes the employee record from Firebase Realtime Database
     */
    public void deleteEmployee(String employeeId, OperationCallback callback) {
        ...
        rootRef.child(PATH_EMPLOYEES).child(employeeId)
                .removeValue()
                .addOnSuccessListener(aVoid -> {
                    Log.d(TAG, "✅ Employee permanently deleted from Firebase: " + employeeId);
                    if (callback != null) callback.onResult(true);
                })
                .addOnFailureListener(e -> {
                    Log.e(TAG, "❌ Failed to permanently delete employee from Firebase: " + employeeId, e);
                    if (callback != null) callback.onResult(false);
                });
    }
